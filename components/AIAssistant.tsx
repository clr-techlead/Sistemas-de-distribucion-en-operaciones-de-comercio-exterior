...
const AIAssistant: React.FC = () => {
  ...
  return (
    <>
      ...
      <div role="dialog" aria-modal="true" aria-label="Logistics Hub AI assistant" className={`fixed top-0 right-0 h-full bg-white/95 z-40 transition-all duration-500 w-full sm:w-[450px] flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {loading && <div role="status" aria-live="polite" className="flex justify-start"><div className="bg-slate-100 p-4 rounded-3xl text-sm text-slate-500">Thinking...</div></div>}
          <div ref={chatEndRef} />
        </div>
